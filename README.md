...
## DPI
 ...
 ### Обычный анализ трафика
  ...
  - RST (от 'reset', сброс) — генерируется RST пакет, который отправиться к клиенту и он выдаст ошибку об сбросе соединения (для Chrome ERR_CONNECTION_RESET)

  - FIN ('final', конец) — генерируется FIN пакет, который отправиться к клиенту и клиент будет обязан закрыть соединение с сервером
